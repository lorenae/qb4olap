...
; 
atom
: url=STRING RELOPER value=STRING 
...
(
  ...
  |{measures.containsKey($url.text) &&( isInteger($value.text) || isDecimal($value.text))}? 
                            {$prog::foundMeasures.put($url.text, "?measure_"+measCounter++);} ->^( RELOPER $url $value)
  |{levels.containsValue($url.text)}? {$prog::foundLevels.put($url.text, "?level_"+levelCounter++);}->^(RELOPER $url $value)
 )
